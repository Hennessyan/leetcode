    ...
    public String alienOrder(String[] words) {
        ...
        for(int i = 0; i < len - 1; i++) {
            ...
            for(int j = 0; j < Math.min(ch1.length, ch2.length); j++) {
                if(ch1[j] != ch2[j]) {
                    if(map.computeIfAbsent(ch1[j], x -> new HashSet<>()).add(ch2[j])) { //注意这里  ["ac","ad", "zc", "zd"]
                        indegree[ch2[j] - 'a']++;
                    }
                    break;
                }
    ...
    public String alienOrder1(String[] words) {
        ...
        for(int i = 1; i < words.length; i++) {
            ...
            for(int j = 0; j < Math.min(w1.length(), w2.length()); j++) {
                ...
                char b = w2.charAt(j);
                if(a != b) {
                    map.get(b).add(a);   // pay attention to the order here
                    break;
                }
            }
        }
...
//        if(sb.length() != map.size()) {   // do not need this condition because of l78-82
//            return "";
//        }
        return sb.toString();
    }
